...
def get_name_id(file, fileName):
    ...
    if line0[0] != "#" or line1[0] != "#":
        failwithmessage(
            "The first two lines of your python files ("
            + fileName
            + ") should be single line comments with your names and netids."
        )
    ...
    if len(names) == 0:
        failwithmessage(
            "No name(s) found in the first line of your python file "
            + fileName
            + ". \nPlease make sure that the first line is a single line comment with your name(s)."
        )
    if len(netids) == 0:
        failwithmessage(
            "No netid(s) found in the second line of your python file "
            + fileName
    ...
    if "," in names and not "," in netids:
        ...
        failwithmessage(
            "Looks like there are two names in the first line of your python file ("
            + fileName
            + "), but only one net ID in the second line. \nPlease make sure that both lines have the same number of names and net IDs."
        )

    if "," not in names and "," in netids:
        failwithmessage(
            "Looks like there is one name in the first line of your python file ("
            + fileName
            + "), but two net IDs in the second line. \nPlease make sure that both lines have the same number of names and net IDs."
        )
    ...
    if "," in names:
        ...
        if len(names[0].strip()) == 0 or len(names[1].strip()) == 0:
            failwithmessage(
                "Looks like there is a comma in the first line of your python file ("
                + fileName
                + "), but at least one of the names is empty. \nPlease make sure that both lines have the same number of names and net IDs."
            )
    ...
    if "," in netids:
        ...
        if len(netids[0].strip()) == 0 or len(netids[1].strip()) == 0:
            failwithmessage(
                "Looks like there is a comma in the second line of your python file ("
                + fileName
                + "), but at least one of the net IDs is empty. \nPlease make sure that both lines have the same number of names and net IDs."
            )
    ...
    for function_name in function_names:
        if not hasattr(module_object, function_name):
            failwithmessage(
                ...
                + "' in "
                + file_name
                + " file. Are you sure it exists?"
            )
        else:
            infowithmessage(
                "Found function '" + function_name + "' in " + file_name + "."
            )
...
def check_submission():
    ...
    if not os.path.exists(os.path.join(cwd, "final_submission.zip")):
        failwithmessage(
            "'final_submission.zip' not found in current directory  Are you sure it exists?"
        )
    else:
    ...
    for file in python_files:
        if not os.path.exists(os.path.join(cwd, "__temp", file)):
            ...
            failwithmessage(
                "Could not find '"
                + file
                + "' in your submission. Are you sure it exists?"
            )
    ...
    for file in python_files:
        ...
        if file == "data_exploration.py":
            ...
            check_module_for_functions(
                data_exploration,
                ["stringify_labeled_doc", "validate_ner_sequence"],
                file,
            )
        ...
        if file == "models.py":
            ...
            if not hasattr(models, "HMM"):
                failwithmessage(
                    "Could not find class 'HMM' in "
                    + file
                    + ". Are you sure it exists?"
                )
            if not hasattr(models, "MEMM"):
                failwithmessage(
                    "Could not find class 'MEMM' in "
                    + file
                    + ". Are you sure it exists?"
                )
        ...
        if file == "helpers.py":
            ...
            check_module_for_functions(
                helpers, ["handle_unknown_words", "apply_smoothing"], file
            )
    ...
    remove_temp_dir()
    successwithmessage(
        "Submission check complete-- it looks like all your files are in order! This is no guarantee."
    )
