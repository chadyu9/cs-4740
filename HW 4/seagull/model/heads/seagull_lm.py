...
class SeagullLM(Module):
    ...
    def forward(
        ...
        # TODO-6.2
        # Obtain the output through the Seagull transformer.
        ...
        logits = (
            torch.matmul(
                all_outputs[-1],
                torch.t(self.seagull.embedding.token_embedding.weight.data),
            )
            if self.weight_tying
            else self.lm_head(all_outputs[-1])
        )

        # Return the logits and the seagull output
    ...
    def talk(
        ...
        def _get_updated_sequence_lengths(
            ...
            if len(eos_token_batch_idxs) > 0:
                new_eos_token_batch_idxs = list(
                    set(eos_token_batch_idxs) - modified_batch_idxs_
                )
                all_sample_sequence_lengths_[new_eos_token_batch_idxs] = (
                    current_generation_length - 1
                )
        ...
        input_ids = (
            input_ids[:, -self._max_positions :]
            if input_ids.shape[1] > self._max_positions
            else input_ids
        )
        position_ids = (
            torch.arange(input_ids.shape[1]).expand(num_samples, -1).to(self.device)
        )

        all_input_ids = input_ids
        all_sample_sequence_lengths = torch.ones(num_samples, dtype=torch.long) * (
        ...
        for _ in range(max_new_tokens):
            ...
            if top_k is not None:
                top_k_vals, _ = torch.topk(
                    lm_logits, k=top_k, dim=-1, largest=True, sorted=True
                )
                lm_logits[lm_logits < top_k_vals[:, [-1]]] = -torch.finfo(
                    lm_logits.dtype
                ).max
            ...
            if eos_token_id is not None:
                (
                    all_sample_sequence_lengths,
                    modified_batch_idxs,
                ) = _get_updated_sequence_lengths(
        ...
        if eos_token_id is not None:
            all_input_ids = [
                seq[: all_sample_sequence_lengths[seq_idx]]
                for seq_idx, seq in enumerate(all_input_ids)
            ]
