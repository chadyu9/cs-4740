...
class MultiHeadAttention(Module):
    def __init__(
        ...
        ), f"embedding_dim: {embedding_dim} not divisible by num_heads: {num_heads}"
        ...
        causal_mask = (
            torch.ones((max_positions, max_positions), dtype=torch.bool).triu(1)
            if causal
            else None
        )
    ...
    @staticmethod
    def _make_numerically_stable(
        similarities: torch.Tensor, softmax_dim: int
    ) -> torch.Tensor:
        return similarities - similarities.amax(dim=softmax_dim, keepdim=True).detach()

    def _update_cache(
        self, tensor: torch.Tensor, cache: Optional[torch.Tensor]
    ) -> torch.Tensor:
        ...
        if self.causal_mask is not None:
            ...
            if query_seq_length != 1:  # ignore when kv-cache in effect
                similarities.masked_fill_(
                    self.causal_mask[:query_seq_length, :value_seq_length], neg_inf
                )
        return similarities
    ...
    def forward(
        ...
        query, key, value = self.qkv_transform(input_embeddings).split(
            split_size=self.embedding_dim, dim=-1
        )
        # b: batch_size, l: max_length, h: num_heads, d: head_dim
        query, key, value = map(
            lambda tensor: rearrange(tensor, "b l (h d) -> b h l d", h=self.num_heads),
            (query, key, value),
        )
        if self.use_rope:
            ...
            start_pos = (
                0
                if (not use_kv_cache or self.k_cache is None)
                else self.k_cache.shape[-2]
            )
            start_pos = (
                self.max_positions - 1 if start_pos >= self.max_positions else start_pos
            )
            ...
            query, key = apply_rope(
                query=query, key=key, cos_vals=cos_vals, sin_vals=sin_vals
            )
        ...
        masked_attn_probs = self.masked_attention_probs(
            query=query, key=key, padding_mask=padding_mask
        )
